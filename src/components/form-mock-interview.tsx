    ...
    try {
      ...
      if (initialData) {
        if (isValid) {
          ...
          setAiQuestions(aiResult); // Set to state
        ...
        <div className="mt-8 w-full">
          ...
          <div className="space-y-4">
            {aiQuestions.map((q, i) => (
              <div
                key={i}
                className="bg-muted p-4 rounded-lg border shadow-sm space-y-2"
              >
                <p className="font-medium">Q{i + 1}. {q.question}</p>
                <p className="text-sm text-muted-foreground">Ans: {q.answer}</p>
              </div>
            ))}
          </div>
        </div>
