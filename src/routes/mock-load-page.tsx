...
export const MockLoadPage = () => {
  ...
  useEffect(() => {
    // Simulate Gemini tip suggestion
    const geminiTips = [
      "Try to follow the STAR method (Situation, Task, Action, Result).",
      ...
      "Show enthusiasm and confidence in your tone."
    ];
    const randomTip = geminiTips[Math.floor(Math.random() * geminiTips.length)];
    setAiTip(randomTip);
  }, []);
    ...
    <div className="flex flex-col w-full gap-8 py-5">
      <div className="flex items-center justify-between w-full gap-2">
        ...
        <Link to={`/generate/interview/${interviewId}/start`}>
          <Button size={"sm"}>
            Start <Sparkles className="ml-1 h-4 w-4" />
          </Button>
        </Link>
      </div>
      ...
      <div className="flex items-center justify-center w-full h-full">
        <div className="w-full h-[400px] md:w-96 flex flex-col items-center justify-center border p-4 bg-gray-50 rounded-md">
          ...
          <p className="text-sm text-muted-foreground mt-4 text-center">
            {isWebCamEnabled ? "Webcam is enabled and ready." : "Click below to enable your webcam."}
          </p>
        </div>
      </div>

      <div className="flex items-center justify-center">
        <Button onClick={() => setIsWebCamEnabled(!isWebCamEnabled)} variant={isWebCamEnabled ? "secondary" : "default"}>
          {isWebCamEnabled ? "Disable Webcam" : "Enable Webcam"}
        </Button>
      </div>

      <div className="text-sm text-gray-600 text-center pt-4">
        <span className="font-semibold">Gemini AI Tip:</span> {aiTip}
      </div>
    </div>
  );
};
